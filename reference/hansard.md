...
per request in some instances, though this has been inconsistent in
testing.

## Details

The API itself is still in beta, and only about half of the planned
datasets are currently available. The package name is optimistic, as the
...
will return the same result. This is because function names are taken
from the specific API on <http://explore.data.parliament.uk/>, but they
are often not very informative and could clash with functions in other
packages (e.g.
[`bills()`](https://ropengov.github.io/hansard/reference/bills.md) is
not a term unique to the British parliament).

For more details please see the
[vignette](https://docs.evanodell.com/hansard/articles/introduction), or
the API documentation on <http://explore.data.parliament.uk/>.
